...
class CodeEditor(QPlainTextEdit):
    def __init__(
        ...
        # Connect cursor position changed signal to highlight matching brackets
    ...
    def _highlight_matching_brackets(self):
        ...
        try:
            match_block = self._doc.findBlock(match_pos)
            ...
            match_byte = self._doc.point_to_byte(Point(match_row, match_byte_col))
        except (IndexError, ValueError):
            self.setExtraSelections(extra_selections)
            return

        # Get the node at the match position
        node = self.tree_manager.get_node_at_point(match_byte)
        if node is None:
            self.setExtraSelections(extra_selections)
            return
        ...
        if match_char in quote_chars:
            ...
            try:
                string_start_char = self._doc.byte_to_char(string_node.start_byte)
                string_end_char = self._doc.byte_to_char(string_node.end_byte)
            except (IndexError, ValueError):
                self.setExtraSelections(extra_selections)
                return

            # Get the actual string text to determine quote length (handle triple quotes)
            string_cursor = self.textCursor()
        ...
        try:
            node_start_char = self._doc.byte_to_char(node.start_byte)
            node_end_char = self._doc.byte_to_char(node.end_byte)
            match_start_char = self._doc.byte_to_char(matching_node.start_byte)
            match_end_char = self._doc.byte_to_char(matching_node.end_byte)
        except (IndexError, ValueError):
            self.setExtraSelections(extra_selections)
            return
